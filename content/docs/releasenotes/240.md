...
+ Adds new image count summaries to Image Analysis pages
+ Adds warning during "Analyze Repo" workflow if repo to be added has a lot of images, allowing users to cancel operation before the analysis is requested to avoid unintentional workload.
+ Adds "Whats New" message on initial login and available in the "About Enterprise Client" selection from Account dropdown in top right of screen.   
+ Adds "Binary" content type in Image Contents tab
+ Adds "Golang" content type in Image Contents tab
+ Adds "Malware" content type in Image Contents tab
+ Adds ability to cancel all pending analyses from a single repository
+ Adds download for report preview data as JSON and CSV
+ Adds custom control for relative time filters in reporting
...
### Fixes
+ Fixes some table cell truncations for image digests and other fields in the Image Analysis tab
+ Fixes changelog showing entries when no change is apparent
+ Fixes re-analysis of images that failed analysis
+ Fixes version mismatch after container restart
+ Fixes results shown after whitelisting an item with an inactive bundle 
