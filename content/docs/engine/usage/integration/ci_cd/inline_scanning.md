...
`curl -s https://ci-tools.anchore.io/inline_scan-v0.5.0 | bash -s -- -p alpine:latest`
...
`curl -s https://ci-tools.anchore.io/inline_scan-v0.5.0 | bash -s -- [options] IMAGE_NAME(s)`
...
`curl -s https://ci-tools.anchore.io/inline_scan-v0.5.0 | bash -s -- -p -r alpine:latest ubuntu:latest centos:latest`
...
curl -s https://ci-tools.anchore.io/inline_scan-v0.5.0 | bash -s -- -f -d Dockerfile -b .anchore-policy.json example-image:latest
```
...
curl -s https://ci-tools.anchore.io/inline_scan-v0.5.0 | bash -s -- -v ./images -t 500
```
...
jobs:
  build_scan_image:
    ...
    - run:
        ...
        command: |
          apk add curl bash
          curl -s https://ci-tools.anchore.io/inline_scan-v0.5.0 | bash -s -- -r "${IMAGE_NAME}:ci"
    - run:
...
container_build:
  ...
  - apk add bash curl 
  - curl -s https://ci-tools.anchore.io/inline_scan-v0.5.0 | bash -s -- -r -t 500 "$IMAGE_NAME"
...
- name: build-scan
  ...
  command: sh -c 'apk add bash curl &&
    ...
    curl -s https://ci-tools.anchore.io/inline_scan-v0.5.0 | bash -s -- -f -b .anchore_policy.json "${IMAGE_NAME}:ci" &&
    echo "$DOCKER_PASS" | docker login -u "$DOCKER_USER" --password-stdin &&
    docker tag "${IMAGE_NAME}:ci" "${IMAGE_NAME}:${IMAGE_TAG}" &&
    docker push "${IMAGE_NAME}:${IMAGE_TAG}"'
```
    ...
    stages {
        ...
        stage('Scan') {
            steps {        
                ...
                sh 'curl -s https://ci-tools.anchore.io/inline_scan-v0.5.0 | bash -s -- -d Dockerfile -b .anchore_policy.json ${IMAGE_NAME}:ci'
            }
        }
        stage('Push Image') {
            steps {
...
script:
  - docker build -t "${IMAGE_NAME}:ci" .
  - curl -s https://ci-tools.anchore.io/inline_scan-v0.5.0 | bash -s -- "${IMAGE_NAME}:ci"
  - echo "$DOCKER_PASS" | docker login -u "$DOCKER_USER" --password-stdin
  - docker tag "${IMAGE_NAME}:ci" "${IMAGE_NAME}:${IMAGE_TAG}"
  - docker push "${IMAGE_NAME}:${IMAGE_TAG}"
```
...
phases:
  ...
  post_build:
    commands:
      - curl -s https://ci-tools.anchore.io/inline_scan-v0.5.0 | bash -s -- ${IMAGE_NAME}:${IMAGE_TAG}
      - echo $DOCKER_PASS | docker login -u $DOCKER_USER --password-stdin
      - docker push ${IMAGE_NAME}:${IMAGE_TAG}
```